...
mavlink.MAV_AUTOPILOT_ENUM_END = 19 // 
...
mavlink.MAV_COMP_ID_AUTOPILOT1 = 1 // 
mavlink.MAV_COMP_ID_CAMERA = 100 // 
mavlink.MAV_COMP_ID_CAMERA2 = 101 // 
mavlink.MAV_COMP_ID_CAMERA3 = 102 // 
mavlink.MAV_COMP_ID_CAMERA4 = 103 // 
mavlink.MAV_COMP_ID_CAMERA5 = 104 // 
mavlink.MAV_COMP_ID_CAMERA6 = 105 // 
...
mavlink.MAV_SYS_STATUS_SENSOR_ENUM_END = 33554433 // 
...
mavlink.MAV_CMD_REQUEST_STORAGE_INFORMATION = 525 // WIP: Request storage information (STORAGE_INFORMATION). Use the
                        // command's target_component to target a
                        // specific component's storage.
mavlink.MAV_CMD_STORAGE_FORMAT = 526 // WIP: Format a storage medium. Once format is complete, a
...
mavlink.MAV_CMD_RESET_CAMERA_SETTINGS = 529 // WIP: Reset all camera settings to Factory Default
mavlink.MAV_CMD_SET_CAMERA_MODE = 530 // Set camera running mode. Use NAN for reserved values.
...
mavlink.MAV_CMD_REQUEST_CAMERA_IMAGE_CAPTURE = 2002 // WIP: Re-request a CAMERA_IMAGE_CAPTURE packet. Use NAN for reserved
                        // values.
...
mavlink.MAV_CMD_VIDEO_STOP_CAPTURE = 2501 // Stop the current video capture (recording). Use NAN for reserved
                        // values.
mavlink.MAV_CMD_VIDEO_START_STREAMING = 2502 // WIP: Start video streaming
mavlink.MAV_CMD_VIDEO_STOP_STREAMING = 2503 // WIP: Stop the current video streaming
mavlink.MAV_CMD_REQUEST_VIDEO_STREAM_INFORMATION = 2504 // WIP: Request video stream information (VIDEO_STREAM_INFORMATION)
...
mavlink.MAV_CMD_ARM_AUTHORIZATION_REQUEST = 3001 // Request authorization to arm the vehicle to a external entity, the arm
                        // authorizer is resposible to request all
...
mavlink.MAV_CMD_CONDITION_GATE = 4501 // WIP: Delay mission state machine until gate has been reached.
...
// SERIAL_CONTROL_DEV
mavlink.SERIAL_CONTROL_DEV_TELEM1 = 0 // First telemetry drone_port
mavlink.SERIAL_CONTROL_DEV_TELEM2 = 1 // Second telemetry drone_port
mavlink.SERIAL_CONTROL_DEV_GPS1 = 2 // First GPS drone_port
mavlink.SERIAL_CONTROL_DEV_GPS2 = 3 // Second GPS drone_port
...
mavlink.SERIAL_CONTROL_FLAG_EXCLUSIVE = 4 // Set if access to the serial drone_port should be removed from whatever
                        // driver is currently using it, giving
                        // exclusive access to the SERIAL_CONTROL
                        // protocol. The drone_port can be handed back by
                        // sending a request without this flag set
mavlink.SERIAL_CONTROL_FLAG_BLOCKING = 8 // Block on writes to the serial drone_port
mavlink.SERIAL_CONTROL_FLAG_MULTI = 16 // Send multiple replies until drone_port is drained
...
mavlink.MAV_MISSION_TYPE_FENCE = 1 // Specifies GeoFence area(s). Items are MAV_CMD_FENCE_ GeoFence items.
...
mavlink.CAMERA_CAP_FLAGS_ENUM_END = 33 // 
...
mavlink.MAVLINK_MSG_ID_SET_VIDEO_STREAM_SETTINGS = 270
...
is currently active or not and if an emergency occured. During the
...
to measure the system latencies, including serial drone_port, radio modem
and UDP connections.
...
(100%) 10000. Channels that are inactive should be set to UINT16_MAX.
                ...
                time_boot_ms              : Timestamp (milliseconds since system boot) (uint32_t)
                drone_port                      : Servo output drone_port (set of 8 outputs = 1 drone_port). Most MAVs will just use one, but this allows for more than 8 servos. (uint8_t)
...
mavlink.messages.rc_channels_scaled.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.time_boot_ms, this.chan1_scaled, this.chan2_scaled, this.chan3_scaled, this.chan4_scaled, this.chan5_scaled, this.chan6_scaled, this.chan7_scaled, this.chan8_scaled, this.fc_port, this.rssi]));
}
...
mavlink.messages.rc_channels_raw.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.time_boot_ms, this.chan1_raw, this.chan2_raw, this.chan3_raw, this.chan4_raw, this.chan5_raw, this.chan6_raw, this.chan7_raw, this.chan8_raw, this.fc_port, this.rssi]));
}
...
1000 microseconds: 0%, 2000 microseconds: 100%.
                ...
                drone_port                      : Servo output drone_port (set of 8 outputs = 1 drone_port). Most MAVs will just use one, but this allows to encode more than 8 servos. (uint8_t)
...
http://qgroundcontrol.org/mavlink/waypoint_protocol.
                ...
                command                   : The scheduled action for the waypoint. see MAV_CMD in common.xml MAVLink specs (uint16_t)
...
/* 
Control a serial drone_port. This can be used for raw access to an onboard
...
WIP: Information about a camera
                ...
                flags                     : CAMERA_CAP_FLAGS enum flags (bitmap) describing camera capabilities. (uint32_t)
...
MAV_CMD_REQUEST_CAMERA_SETTINGS.
                ...
                time_boot_ms              : Timestamp (milliseconds since system boot) (uint32_t)
                mode_id                   : Camera mode (CAMERA_MODE) (uint8_t)

*/
mavlink.messages.camera_settings = function(time_boot_ms, mode_id) {
    ...
    this.fieldnames = ['time_boot_ms', 'mode_id'];
...
mavlink.messages.camera_settings.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.time_boot_ms, this.mode_id]));
}
...
*/
mavlink.messages.video_stream_information = function(camera_id, status, framerate, resolution_h, resolution_v, bitrate, rotation, uri) {
...
mavlink.messages.video_stream_information.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.framerate, this.bitrate, this.resolution_h, this.resolution_v, this.rotation, this.camera_id, this.status, this.uri]));
}
...
*/
mavlink.messages.set_video_stream_settings = function(target_system, target_component, camera_id, framerate, resolution_h, resolution_v, bitrate, rotation, uri) {

    this.format = '<fIHHHBBB230s';
    this.id = mavlink.MAVLINK_MSG_ID_SET_VIDEO_STREAM_SETTINGS;
    this.order_map = [5, 6, 7, 0, 2, 3, 1, 4, 8];
    this.crc_extra = 232;
    this.name = 'SET_VIDEO_STREAM_SETTINGS';

    this.fieldnames = ['target_system', 'target_component', 'camera_id', 'framerate', 'resolution_h', 'resolution_v', 'bitrate', 'rotation', 'uri'];
...
mavlink.messages.set_video_stream_settings.prototype = new mavlink.message;

mavlink.messages.set_video_stream_settings.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.framerate, this.bitrate, this.resolution_h, this.resolution_v, this.rotation, this.target_system, this.target_component, this.camera_id, this.uri]));
}
...
/* 
WIP: Version and capability of protocol version. This message is the
...
*/
mavlink.messages.protocol_version = function(version, min_version, max_version, spec_version_hash, library_version_hash) {
...
after a loss or timeout.
                ...
                param_type                : Parameter type: see the MAV_PARAM_EXT_TYPE enum for supported data types. (uint8_t)
...
Response from a PARAM_EXT_SET message.
                ...
                param_type                : Parameter type: see the MAV_PARAM_EXT_TYPE enum for supported data types. (uint8_t)
                param_result              : Result code: see the PARAM_ACK enum for possible codes. (uint8_t)
...
increment degrees to the right
                ...
                distances                 : Distance of obstacles in front of the sensor starting on the left side. A value of 0 means that the obstacle is right in front of the sensor. A value of max_distance +1 means no obstace is present. A value of UINT16_MAX for unknown/not used. In a array element, each unit corresponds to 1cm. (uint16_t)
...
mavlink.map = {
        ...
        260: { format: '<IB', type: mavlink.messages.camera_settings, order_map: [0, 1], crc_extra: 146 },
        ...
        269: { format: '<fIHHHBB230s', type: mavlink.messages.video_stream_information, order_map: [5, 6, 0, 2, 3, 1, 4, 7], crc_extra: 58 },
        270: { format: '<fIHHHBBB230s', type: mavlink.messages.set_video_stream_settings, order_map: [5, 6, 7, 0, 2, 3, 1, 4, 8], crc_extra: 232 },
